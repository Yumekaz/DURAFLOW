...
func getTestPostgresStore(t *testing.T) *PostgresStore {
	dsn := os.Getenv("DURAFLOW_TEST_POSTGRES_DSN")
	if dsn == "" {
		t.Skip("Skipping Postgres integration tests: DURAFLOW_TEST_POSTGRES_DSN not set")
		return nil
	}
...
func cleanupPostgres(t *testing.T, p *PostgresStore) {
	tables := []string{
		"logs",
		"events",
		"step_states",
	...
	for _, table := range tables {
		_, err := p.db.Exec("TRUNCATE TABLE " + table + " CASCADE")
		if err != nil {
...
func TestPostgresStore_LifecycleAndOperations(t *testing.T) {
...
func TestPostgresStore_WorkersAndLeases(t *testing.T) {
	store := getTestPostgresStore(t)
	if store == nil {
		return
	}
...
func TestPostgresTimersAndCronSchedules(t *testing.T) {
	store := getTestPostgresStore(t)
	if store == nil {
		return
	}
